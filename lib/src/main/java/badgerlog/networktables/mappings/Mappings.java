...
/// Class containing a list of all [Mappings][Mapping] for every registered type and providing methods to find specific mappings.
public final class Mappings {
    ...
    /// Find a [Mapping] from a type class. It is assumed that only one mapping per class type is created, and will throw an error if there are more than one present
    ///
    /// @param type        the [Class] representing
    /// @param <FieldType> the type on the field
    /// @param <NTType>    the [Object] representation of a [NetworkTableType]
    ...
    /// Finds the [NetworkTableType] given a class type
    ///
    /// @param type the type as a [Class]
